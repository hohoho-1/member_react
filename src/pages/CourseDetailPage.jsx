...
function ReviewSection({ courseId, enrollment }) {
  ...
  const fetchMyReview = async () => {
    ...
    if (res.ok) setMyReview(await res.json());
  };
  const openWriteForm = () => { setEditMode(false); setRating(5); setContent(''); setShowForm(true); };
  const openEditForm = () => { setEditMode(true); setRating(myReview.rating); setContent(myReview.content || ''); setShowForm(true); };
  const handleSubmit = async () => {
    if (!content.trim()) { error('후기 내용을 입력해주세요.'); return; }
    setSubmitting(true);
    try {
      const url = editMode ? `/api/courses/${courseId}/reviews/${myReview.id}` : `/api/courses/${courseId}/reviews`;
      const method = editMode ? 'PUT' : 'POST';
      const res = await authFetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rating, content }) });
      if (res.ok) { setShowForm(false); await fetchReviews(); await fetchMyReview(); }
      else { const e = await res.json().catch(() => ({})); error(e.message || '오류가 발생했습니다.'); }
  ...
  const handleDelete = async (reviewId) => {
    ...
    if (res.ok) { await fetchReviews(); await fetchMyReview(); }
  };
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 mt-4">
      <div className="flex items-center justify-between mb-4">
        ...
        {canWrite && !myReview && !showForm && (
          <button onClick={openWriteForm} className="px-4 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition-colors">후기 작성</button>
        )}
      </div>
      {showForm && (
        <div className="mb-5 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800">
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{editMode ? '후기 수정' : '후기 작성'}</p>
          <div className="mb-3">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">별점</p>
            <StarRating value={rating} onChange={setRating} size="lg" />
          </div>
          <textarea value={content} onChange={e => setContent(e.target.value)} rows={4} placeholder="수강 후 느낀 점을 자유롭게 작성해 주세요."
          ...
          <div className="flex gap-2 mt-2 justify-end">
            <button onClick={() => setShowForm(false)} className="px-4 py-1.5 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg transition-colors">취소</button>
            <button onClick={handleSubmit} disabled={submitting} className="px-4 py-1.5 bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white rounded-lg text-sm font-medium transition-colors">
      ...
      {isLoggedIn() && !canWrite && !myReview && (
        <div className="text-sm text-gray-400 dark:text-gray-500 mb-4 text-center py-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">수료 완료 후 후기를 작성할 수 있습니다.</div>
      )}
      {reviews.length === 0 ? (
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-blue-600 dark:text-blue-300 font-bold text-sm shrink-0 overflow-hidden">
                    {review.authorAvatar ? <img src={review.authorAvatar} alt="" className="w-full h-full object-cover" /> : review.authorName?.[0]}
                  </div>
                  <div>
                    <div className="flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {review.mine && (<><button onClick={openEditForm} className="text-xs text-blue-500 hover:underline">수정</button><button onClick={() => handleDelete(review.id)} className="text-xs text-red-400 hover:underline">삭제</button></>)}
                  {isAdmin() && !review.mine && <button onClick={() => handleDelete(review.id)} className="text-xs text-red-400 hover:underline">삭제</button>}
                </div>
              </div>
              {review.content && <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 leading-relaxed ml-10">{review.content}</p>}
            </div>
          ))}
        </div>
      )}
...
function QnaSection({ courseId, enrollment }) {
  ...
  const { confirmProps, confirm } = useConfirm();
  const { error } = useToastContext();
  const canWrite = !!enrollment;

  useEffect(() => { fetchQna(); }, [courseId, page]); // eslint-disable-line

  const fetchQna = async () => {
    const res = await authFetch(`/api/courses/${courseId}/qna?page=${page}&size=10`);
    if (res.ok) { const d = await res.json(); setQnaList(d.content); setTotalPages(d.totalPages); }
  };
  const openWriteForm = () => { setEditTarget(null); setTitle(''); setContent(''); setShowForm(true); };
  const openEditForm = (q) => { setEditTarget(q); setTitle(q.title); setContent(q.content); setShowForm(true); };
  const handleSubmit = async () => {
    if (!title.trim() || !content.trim()) { error('제목과 내용을 입력해주세요.'); return; }
    setSubmitting(true);
    try {
      const url = editTarget ? `/api/courses/${courseId}/qna/${editTarget.id}` : `/api/courses/${courseId}/qna`;
      const method = editTarget ? 'PUT' : 'POST';
      const res = await authFetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, content }) });
      if (res.ok) { setShowForm(false); setPage(0); await fetchQna(); }
      else { const e = await res.json().catch(() => ({})); error(e.message || '오류가 발생했습니다.'); }
    } finally { setSubmitting(false); }
  };
  ...
  const handleAnswerSubmit = async (qnaId) => {
    ...
    const res = await authFetch(`/api/courses/${courseId}/qna/${qnaId}/answer`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: answerContent }) });
    if (res.ok) { setAnswerEditId(null); setAnswerContent(''); await fetchQna(); }
  };
  ...
  return (
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        {canWrite && !showForm && (
          <button onClick={openWriteForm} className="px-4 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition-colors">질문하기</button>
        )}
      </div>
      {showForm && (
        <div className="mb-5 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800">
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{editTarget ? '질문 수정' : '질문 작성'}</p>
          ...
          <textarea value={content} onChange={e => setContent(e.target.value)} rows={4} placeholder="질문 내용을 자세히 작성해 주세요."
            className="w-full text-sm border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 resize-none focus:outline-none focus:ring-2 focus:ring-blue-400" />
          <div className="flex gap-2 mt-2 justify-end">
            <button onClick={() => setShowForm(false)} className="px-4 py-1.5 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg transition-colors">취소</button>
            <button onClick={handleSubmit} disabled={submitting} className="px-4 py-1.5 bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white rounded-lg text-sm font-medium transition-colors">
      ...
      {isLoggedIn() && !canWrite && (
        <div className="text-sm text-gray-400 dark:text-gray-500 mb-4 text-center py-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">수강 신청 후 질문을 작성할 수 있습니다.</div>
      )}
      {qnaList.length === 0 ? (
        ...
        <div className="divide-y divide-gray-100 dark:divide-gray-700">
          {qnaList.map(q => (
            <div key={q.id} className="py-3">
              <div className="flex items-start gap-2 cursor-pointer" onClick={() => setOpenId(openId === q.id ? null : q.id)}>
                <span className={`shrink-0 mt-0.5 text-xs font-semibold px-1.5 py-0.5 rounded ${q.answer ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-500'}`}>
                  {q.answer ? '답변완료' : '미답변'}
                </span>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {q.mine && !q.answer && (<><button onClick={e => { e.stopPropagation(); openEditForm(q); }} className="text-xs text-blue-500 hover:underline">수정</button><button onClick={e => { e.stopPropagation(); handleDelete(q.id); }} className="text-xs text-red-400 hover:underline">삭제</button></>)}
                  {isAdmin() && <button onClick={e => { e.stopPropagation(); handleDelete(q.id); }} className="text-xs text-red-400 hover:underline">삭제</button>}
                  <span className="text-gray-400 text-xs">{openId === q.id ? '▲' : '▼'}</span>
                </div>
              </div>
              {openId === q.id && (
                <div className="mt-3 ml-14 space-y-3">
                  <div className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed whitespace-pre-wrap bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">{q.content}</div>
                  {q.answer && (
                    <div className="bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {isAdmin() && (<><button onClick={() => { setAnswerEditId(q.id); setAnswerContent(q.answer.content); }} className="text-xs text-blue-500 hover:underline">수정</button><button onClick={() => handleAnswerDelete(q.id)} className="text-xs text-red-400 hover:underline">삭제</button></>)}
                        </div>
                      </div>
                      {answerEditId === q.id ? (
                        <div>
                          <textarea value={answerContent} onChange={e => setAnswerContent(e.target.value)} rows={3}
                          ...
                          <div className="flex gap-2 mt-1.5 justify-end">
                            <button onClick={() => setAnswerEditId(null)} className="text-xs text-gray-500 border border-gray-200 dark:border-gray-600 rounded px-3 py-1">취소</button>
                            <button onClick={() => handleAnswerSubmit(q.id)} className="text-xs bg-green-500 hover:bg-green-600 text-white rounded px-3 py-1">수정</button>
                          </div>
                        </div>
                      ) : (
                  ...
                  {isAdmin() && !q.answer && answerEditId !== q.id && (
                    <button onClick={() => { setAnswerEditId(q.id); setAnswerContent(''); }}
                      className="text-xs text-green-600 hover:text-green-700 border border-green-200 dark:border-green-700 rounded-lg px-3 py-1.5 transition-colors">+ 답변 작성</button>
                  )}
                  {isAdmin() && !q.answer && answerEditId === q.id && (
                    <div className="bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 rounded-lg p-3">
                      <p className="text-xs font-semibold text-green-700 dark:text-green-300 mb-2">답변 작성</p>
                      <textarea value={answerContent} onChange={e => setAnswerContent(e.target.value)} rows={3} placeholder="답변 내용을 입력하세요."
                        className="w-full text-sm border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 resize-none focus:outline-none focus:ring-2 focus:ring-green-400" />
                      <div className="flex gap-2 mt-1.5 justify-end">
                        <button onClick={() => setAnswerEditId(null)} className="text-xs text-gray-500 border border-gray-200 dark:border-gray-600 rounded px-3 py-1">취소</button>
                        <button onClick={() => handleAnswerSubmit(q.id)} className="text-xs bg-green-500 hover:bg-green-600 text-white rounded px-3 py-1 font-medium">등록</button>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div className="flex justify-center gap-1 mt-4">
          {Array.from({ length: totalPages }, (_, i) => (
            <button key={i} onClick={() => setPage(i)}
              className={`w-7 h-7 rounded text-xs transition-colors ${page === i ? 'bg-blue-500 text-white' : 'bg-white dark:bg-gray-700 text-gray-500 border border-gray-200 dark:border-gray-600 hover:bg-gray-50'}`}>
              {i + 1}
            </button>
          ))}
        </div>
...
function EnrollSection({ course, enrolling, onEnroll }) {
  ...
  const notInRegPeriod = regStart && regEnd ? (today < regStart || today > regEnd) : regStart ? today < regStart : false;
  const btnDisabled = enrolling || isFull || notInRegPeriod;
  const btnText = enrolling ? '신청 중...' : isFull ? '정원 마감' : notInRegPeriod ? '접수 기간이 아닙니다' : '수강 신청하기';

  return (
    <div className="space-y-2">
      {course.price > 0 ? (
        <div className="text-center"><span className="text-2xl font-bold text-gray-800 dark:text-white">{course.price.toLocaleString()}원</span></div>
      ) : (
        <div className="text-center"><span className="text-xl font-bold text-green-600 dark:text-green-400">무료</span></div>
      )}
      <button onClick={onEnroll} disabled={btnDisabled}
        className={`w-full py-2.5 text-white rounded-lg text-sm font-medium transition-colors ${btnDisabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'}`}>
        {btnText}
      </button>
      {course.maxStudents && (
...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    fetchCourse();
    if (isLoggedIn()) { fetchMyEnrollment(); fetchMyWaiting(); }
  }, [courseId]); // eslint-disable-line

  const fetchCourse = async () => {
    setLoading(true);
    const res = await authFetch(`/api/courses/${courseId}`);
    if (res.ok) { const d = await res.json(); setCourse(d); document.title = `${d.title} | GetSmart`; setLiked(d.likedByMe ?? false); setLikeCount(d.likeCount ?? 0); }
  ...
  const fetchMyEnrollment = async () => {
    ...
    if (res.ok) { const d = await res.json(); setEnrollment(d.find(e => String(e.courseId) === String(courseId)) || null); }
  };
  const fetchMyWaiting = async () => {
    const res = await authFetch('/api/courses/my/waiting');
    if (res.ok) { const d = await res.json(); setWaiting(d.find(w => String(w.courseId) === String(courseId)) || null); }
  };
  ...
  const handlePayAndEnroll = async () => {
    ...
    try {
      ...
      tossPayments.requestPayment('카드', {
        amount: course.price, orderId, orderName: course.title, customerName: '수강생',
        successUrl: `${window.location.origin}/payment/success`,
        failUrl: `${window.location.origin}/payment/fail`,
      });
    } catch (e) { error('결제 중 오류가 발생했습니다.'); setEnrolling(false); }
  };
  ...
  const handleEnroll = async () => {
    ...
    if (res.ok) {
      ...
      if (data.paymentRequired) {
        success('수강 신청이 완료되었습니다. 마이페이지에서 결제를 진행해주세요.');
      } else {
      ...
      if (err.message === 'FULL') {
        const ok = await confirm({ title: '정원 초과', message: '현재 정원이 가득 찼습니다.\n대기자 명단에 등록하시겠습니까?', confirmText: '대기 신청', confirmColor: 'blue' });
        if (ok) await handleJoinWaiting();
      } else { error(err.message || '수강 신청에 실패했습니다.'); }
    }
    setEnrolling(false);
  };
  ...
  const handleJoinWaiting = async () => {
    ...
    if (res.ok) { const d = await res.json(); setWaiting(d); success(`대기 ${d.position}번으로 등록되었습니다.`); }
    else { const e = await res.json().catch(() => ({})); error(e.message || '대기 신청에 실패했습니다.'); }
  };
  const handleCancelWaiting = async () => {
    const ok = await confirm({ title: '대기 취소', message: '대기 신청을 취소하시겠습니까?', confirmText: '취소', confirmColor: 'red' });
    if (!ok) return;
    const res = await authFetch(`/api/courses/${courseId}/waiting`, { method: 'DELETE' });
    if (res.ok) { setWaiting(null); success('대기 신청이 취소되었습니다.'); }
  };
  ...
  const handleCancelEnroll = async () => {
    const ok = await confirm({ title: '수강 취소', message: '수강을 취소하시겠습니까?\n진도 및 학습 기록이 모두 삭제됩니다.', confirmText: '수강 취소', confirmColor: 'red' });
    if (!ok) return;
    const res = await authFetch(`/api/courses/${courseId}/enroll`, { method: 'DELETE' });
    if (res.ok) { setEnrollment(null); success('수강이 취소되었습니다.'); }
    else { const e = await res.json().catch(() => ({})); error(e.message || '수강 취소에 실패했습니다.'); }
  };
  const toggleSection = (sectionId) => setOpenSections(prev => ({ ...prev, [sectionId]: !prev[sectionId] }));
  const totalLessons = course?.sections?.reduce((acc, s) => acc + (s.lessons?.length || 0), 0) || 0;
  const lessonTypeIcon = (type) => type === 'VIDEO' ? '🎬' : type === 'QUIZ' ? '📝' : '📄';
  const formatSeconds = (sec) => { if (!sec) return ''; const m = Math.floor(sec / 60); const s = sec % 60; return `${m}:${String(s).padStart(2, '0')}`; };

  if (loading) return <SkeletonCourseDetail />;
  if (!course) return <div className="flex justify-center py-20 text-gray-400">강의를 찾을 수 없습니다.</div>;

  return (
    <>
    <div className="max-w-6xl mx-auto px-4 py-6">
      <button onClick={() => navigate('/courses')} className="text-sm text-gray-500 dark:text-gray-400 hover:text-blue-500 mb-4 flex items-center gap-1 transition-colors">
        ← 강의 목록으로
      </button>
                ...
                {course.sections?.map((section, si) => (
                  <div key={section.id} className="border border-gray-100 dark:border-gray-700 rounded-lg overflow-hidden">
                    <button onClick={() => toggleSection(section.id)}
                      ...
                      <span className="font-medium text-sm text-gray-800 dark:text-gray-200">{si + 1}. {section.title}</span>
                    ...
                    {openSections[section.id] && (
                      <div className="divide-y divide-gray-50 dark:divide-gray-700">
                        {section.lessons?.map((lesson, li) => (
                          <div key={lesson.id}
                            onClick={() => enrollment && !enrollment.paymentRequired && navigate(`/courses/${courseId}/lessons/${lesson.id}`)}
                            className={`flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${
                              enrollment && !enrollment.paymentRequired
                            ...
                            {lesson.durationSeconds && <span className="text-xs text-gray-400">{formatSeconds(lesson.durationSeconds)}</span>}
                            {(!enrollment || enrollment.paymentRequired) && <span className="text-xs">🔒</span>}
                          </div>
                        ))}
                      </div>
                    )}
        ...
        <div className="lg:w-80 lg:shrink-0">
          <div className="lg:sticky lg:top-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 space-y-4">
            ...
            <div>
              ...
              {course.description && <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{course.description}</p>}
            </div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleLike} disabled={liking}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${
                  liked ? 'bg-red-50 dark:bg-red-900/20 border-red-300 dark:border-red-700 text-red-500'
            ...
            {(course.registrationStartDate || course.educationStartDate || course.location || course.instructor) && (
              <div className="space-y-1.5 text-xs text-gray-600 dark:text-gray-400 border-t border-gray-100 dark:border-gray-700 pt-3">
                {course.registrationStartDate && <div className="flex gap-2"><span className="shrink-0 text-gray-400">📅 접수기간</span><span>{course.registrationStartDate} ~ {course.registrationEndDate || '-'}</span></div>}
                {course.educationStartDate && <div className="flex gap-2"><span className="shrink-0 text-gray-400">🎓 교육기간</span><span>{course.educationStartDate} ~ {course.educationEndDate || '-'}</span></div>}
                {course.location && <div className="flex gap-2"><span className="shrink-0 text-gray-400">📍 교육장소</span><span>{course.location}</span></div>}
                {course.instructor && <div className="flex gap-2"><span className="shrink-0 text-gray-400">👨‍🏫 강사</span><span>{course.instructor}</span></div>}
                {course.price > 0
                  ? <div className="flex gap-2"><span className="shrink-0 text-gray-400">💳 수강료</span><span className="font-semibold text-blue-600 dark:text-blue-400">{course.price.toLocaleString()}원</span></div>
                  : <div className="flex gap-2"><span className="shrink-0 text-gray-400">💳 수강료</span><span className="font-semibold text-green-600 dark:text-green-400">무료</span></div>
                }
                {course.maxStudents && (
            ...
            <div className="border-t border-gray-100 dark:border-gray-700 pt-3">
                ...
                <div className="space-y-2">
                  <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg px-4 py-3">
                    ...
                    {enrollment.paymentDeadline && (
                      <p className="text-xs text-amber-500 mt-1">마감: {new Date(enrollment.paymentDeadline).toLocaleString('ko-KR')}</p>
                    )}
                  </div>
                  <button onClick={() => navigate('/mypage?tab=courses')}
              ...
              {enrollment && !enrollment.paymentRequired && (
                <div className="space-y-2">
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div className="bg-blue-500 h-2 rounded-full transition-all" style={{ width: `${enrollment.progressRate}%` }} />
                  </div>
                  {(enrollment.completed || enrollment.isCompleted) && (
                    <div className="flex items-center gap-2 text-green-600 text-sm font-medium mt-2">
                      🎓 수료 완료!
                      <button onClick={() => navigate('/mypage?tab=certificates')} className="text-xs text-blue-500 hover:underline">수료증 보기</button>
                    </div>
                  )}
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (resumeLessonId) navigate(`/courses/${courseId}/lessons/${resumeLessonId}`);
                      else { const first = course.sections?.[0]?.lessons?.[0]; if (first) navigate(`/courses/${courseId}/lessons/${first.id}`); }
                    }}
              ...
              {/* 케이스 4: 미수강 */}
              {!enrollment && !waiting && (
                <EnrollSection course={course} enrolling={enrolling} onEnroll={handleEnroll} />
              )}

      ...
      <div className="flex justify-between items-start gap-2">
        <div className="flex-1">
          ...
          {isEditing ? (
            ...
            <p className={`text-sm whitespace-pre-wrap ${comment.deleted ? 'text-gray-400 italic' : 'text-gray-600'}`}>
              {comment.content}
            </p>
          )}
          ...
          {isReplying && (
            <div className="flex gap-2 mt-2">
              ...
              <div className="flex flex-col gap-1">
                ...
                <button onClick={() => { setReplyingToId(null); setReplyInput(''); }}
                  className="px-3 py-1 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg text-xs transition-colors">
                  취소
                </button>
              </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            {payload && (
              <button onClick={() => {
                ...
                className="px-2 py-1 text-xs text-green-500 hover:bg-green-50 rounded-lg transition-colors">
                답글
              </button>
            )}
...
export default function PostDetailPage() {
  ...
  const handleReply = async (parentId) => {
    ...
    if (res.ok) {
      ...
      const data = await res.json();
      setCommentError(data.message || '답글 작성에 실패했습니다.');
    }
  };
